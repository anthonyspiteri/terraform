...
resource "vsphere_virtual_machine" "TPM03-K8-NODE2" {
  # VM placement #
  name             = "${var.vsphere_vm_name_k8n2}"
  ...
  clone {
    ...
    customize {
      linux_options {
        host_name = "${var.vsphere_vm_name_k8n2}"
        domain    = "${var.vsphere_domain}"
        #time_zone = "${var.vsphere_time_zone}"
      }

      network_interface {
        ipv4_address = "${var.vsphere_ipv4_address_k8n2}"
